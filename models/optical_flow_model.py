...
def test_optical_flow_model():
    ...
    # 创建模型
    model = create_optical_flow_model(config)
    ...
    print(f"冻结backbone后可训练参数数量: {trainable_params_frozen}")
    
    model.unfreeze_backbone()
    trainable_params_unfrozen = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"解冻backbone后可训练参数数量: {trainable_params_unfrozen}")


if __name__ == "__main__":
    test_optical_flow_model()

...
def test_fusion_model():
    ...
    # 测试基础融合模型
    print("测试基础融合模型:")
    model = create_fusion_model(config, advanced=False)
    
    optical_features = torch.randn(batch_size, 2)
    landmark_features = torch.randn(batch_size, 2)
    
    with torch.no_grad():
        output = model(optical_features, landmark_features)
    
    print(f"光流特征形状: {optical_features.shape}")
    print(f"关键点特征形状: {landmark_features.shape}")
    ...
    advanced_config['use_attention'] = True
    
    advanced_model = create_fusion_model(advanced_config, advanced=True)
    
    with torch.no_grad():
        advanced_output = advanced_model(optical_features, landmark_features)
    
    print(f"高级版输出形状: {advanced_output.shape}")
    print(f"高级版模型参数数量: {sum(p.numel() for p in advanced_model.parameters())}")

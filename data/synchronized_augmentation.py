...
def test_synchronized_augmentation():
    ...
    # 创建增强器
    augmenter = create_synchronized_transforms(config)
    
    # 测试数据
    test_image = np.random.randint(0, 255, (224, 224, 3), dtype=np.uint8)
    test_expr_landmarks = np.random.randn(146, 2).astype(np.float32) * 50 + 112  # 中心在图像中心附近
    test_base_landmarks = np.random.randn(146, 2).astype(np.float32) * 50 + 112
    ...
    for i in range(3):
        ...
        aug_image, aug_expr, aug_base = augmenter(
            test_image, test_expr_landmarks, test_base_landmarks, training=True
        )
        
        print(f"原始图像形状: {test_image.shape}")
        print(f"增强后图像形状: {aug_image.shape}")
        print(f"原始表情关键点范围: x[{test_expr_landmarks[:, 0].min():.1f}, {test_expr_landmarks[:, 0].max():.1f}], "
              f"y[{test_expr_landmarks[:, 1].min():.1f}, {test_expr_landmarks[:, 1].max():.1f}]")
        print(f"增强后表情关键点范围: x[{aug_expr[:, 0].min():.1f}, {aug_expr[:, 0].max():.1f}], "
              f"y[{aug_expr[:, 1].min():.1f}, {aug_expr[:, 1].max():.1f}]")
    
    # 验证模式测试
    print("\n验证模式测试:")
    val_image, val_expr, val_base = augmenter(
        test_image, test_expr_landmarks, test_base_landmarks, training=False
    )
    print(f"验证模式图像形状: {val_image.shape}")
    print(f"验证模式关键点形状: {val_expr.shape}")
